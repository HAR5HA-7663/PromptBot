...
def get_cluster_points(main_query, num_points):
        ...
        for i in range(3):
            ...
            if drive_succeeded==True:
                ...
                for j in range(3):
                    ...
                    # Prepare the request
                    rospy.logwarn("attempting num_points: %s", new_num_points)
                    response = get_cluster(main_query=main_query, num_points=new_num_points)
                    points_received=len(response.pts)
...
def main():
    ...
    parser.add_argument('num_points', type=int, help="Number of points to request")

    args = parser.parse_args()

    # Retrieve initial cluster points once
    global points
    points = get_cluster_points(args.main_query,args.num_points)
    if not points:
